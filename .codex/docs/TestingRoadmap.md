...
| Eclipse event scheduler still depends on live Core registries | [EclipseService](../../Services/EclipseService.cs) | _Gap — add_ `EclipseServiceTests.cs` once `Core.ServerGameManager` and `Core.SystemService` registries can be replaced with injectable facades or fake schedulers that run deterministically in tests; avoid booting the live runtime | ⛔ Not covered |
...
| Familiar dismissal routines clean up orphaned familiars and yield updates while offline | [FamiliarService.DisabledFamiliarPositionUpdateRoutine](../../Services/FamiliarService.cs) / [FamiliarService.DestroyFamiliars](../../Services/FamiliarService.cs) | _Gap — add_ `FamiliarServiceDismissalTests.cs` once the service accepts injectable entity/query providers plus a coroutine host so the routines can run without booting `Core.Initialize()` | ⛔ Not covered |
...
| Quest orchestration still relies on `Core`'s live world queries | [QuestService](../../Services/QuestService.cs) | _Gap — add_ `QuestServiceTests.cs` once injectable quest entity/query providers exist; we need seams (for example, a fake quest context akin to `FakeVBloodEntityContext`) before coverage can avoid `Core.Initialize()` spinning up the PAL host and crashing. | ⛔ Not covered |
| Battle matchmaking and raid scheduling still require the live server world | [BattleService](../../Services/BattleService.cs) | _Gap — add_ `BattleServiceTests.cs` when we can inject battle entity/query providers or a factory bootstrapper seam; we need those seams before tests can cover the flows without calling into `Core`'s world bootstrap (which currently crashes the PAL host). Mirror the fake-context/bootstrapped seams used by `EliteShardBearerBootstrapperTests`. | ⛔ Not covered |

Testing `DisabledFamiliarPositionUpdateRoutine` and `DestroyFamiliars` without the live bootstrap still requires a seam for resolving entities/queries (similar to `FakeVBloodEntityContext`) and a coroutine host we can drive deterministically inside tests. Until those hooks exist, the service forces `Core.Initialize()` just to acquire the DOTS runtime, which keeps dismissal coverage out of reach.

We also lack a fake familiar context today, so contributors must **not** call into the live DOTS runtime when exercising dismissal flows. Extend the existing fake contexts—or add a dedicated familiar entity/query provider—before authoring tests for these routines.

`EclipseService` currently reaches directly into `Core.ServerGameManager` and `Core.SystemService` to register eclipse state, schedule ticks, and fan out cleanup callbacks. Those dependencies spin up the production schedulers unless we introduce injectable registries or a fake scheduler host, so the scenario remains uncovered. Plan to carve out those seams first rather than invoking `Core.Initialize()` inside tests.
...
* Prefer the `EliteShardBearerBootstrapper`/`ShardBearerResetService` seam along with `FakeVBloodEntityContext` when exercising shard reset paths—invoking `Core.Initialize()` still spins up the production DOTS services and will crash on the PAL host.
