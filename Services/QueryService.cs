...
internal static class QueryService
{
    ...
    static QueryService()
    {
        World server = WorldUtility.FindServerWorld();
        EntityManager = server.EntityManager;
        _updateBuffsBufferDestroyQuery = ModifyEntityQuery(
            server.GetExistingSystemManaged<UpdateBuffsBuffer_Destroy>().EntityQueries[0],
            [ComponentType.ReadOnly(Il2CppType.Of<PrefabGUID>())]
        );

        _buffSpawnServerQuery = ModifyEntityQuery(
            server.GetExistingSystemManaged<BuffSystem_Spawn_Server>().EntityQueries[0],
            [ComponentType.ReadOnly(Il2CppType.Of<PrefabGUID>())]
        );

        _scriptSpawnServerQuery = ModifyEntityQuery(
            server.GetExistingSystemManaged<ScriptSpawnServer>().EntityQueries[0],
